    ...
    private func updateThemeAndStrings() {
        ...
        if case .generic = self.mode {
            if let isEmpty = self.isEmpty, isEmpty {
                ...
                if self.editingMode {
                    self.navigationItem.rightBarButtonItem = UIBarButtonItem(title: "___done", style: UIBarButtonItem.Style(rawValue: 2) ?? .plain, target: self, action: #selector(self.donePressed))
                } else {
                    ...
                    if case .generic = strongSelf.mode {
                        if empty {
                            ...
                            if strongSelf.editingMode {
                                strongSelf.navigationItem.rightBarButtonItem = UIBarButtonItem(title: strongSelf.presentationData.strings.Common_Done, style: UIBarButtonItem.Style(rawValue: 2) ?? .plain, target: strongSelf, action: #selector(strongSelf.donePressed))
                            } else {
